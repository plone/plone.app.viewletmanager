...
class ViewletSettingsStorageNodeAdapter(XMLAdapterBase):
    ...
    def getSkinSelections(self):
        if not hasattr(self, '_skin_selections'):
            self._skin_selections = [skin.token for skin in
                getUtility(IVocabularyFactory, 'plone.app.vocabularies.Skins')(
                    self.environ.getSite())]
        return self._skin_selections
    ...
    def _importNode(self, node):
        ...
        for child in node.childNodes:
            ...
            if skinname == '*':
                for skinname in self.getSkinSelections():
                    values = []
